...
for i in range(len(hands)):
    ...
    if fiveKind(hand) :
        ...
        if(countJ == 1 or countJ == 4):
            hands[i][2] = 7 # five of a Kind
        else:
            hands[i][2] = 6
    elif fullHouse(hand):
        if(countJ == 2 or countJ == 3):
            hands[i][2] = 7 # five of a Kind
        else:
            hands[i][2] = 5
    elif threeKind(hand):
        if(countJ == 3 or countJ == 1):
            hands[i][2] = 6 # four of a kind
        else:
            hands[i][2] = 4
    elif twoPair(hand):
        if(countJ == 1):
            hands[i][2] = 5 # full house
        elif(countJ == 2):
            hands[i][2] = 6 # four of a kind
        else:
            hands[i][2] = 3
    elif onePair(hand):
        if(countJ == 1 or countJ == 2):
            hands[i][2] = 4 # three of a kind
        else:
            hands[i][2] = 2
    else:
        if(countJ == 1):
            hands[i][2] = 2 # one pair
        else:
            hands[i][2] = 1
